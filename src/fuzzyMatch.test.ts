...
describe("checkAnswer", () => {
  describe("exact matches", () => {
    it("returns match for exact main name", () => {
      expect(checkAnswer("Norge", "Norge", allNorwegianNames)).toEqual([
        "match",
        "Norge",
      ]);
      expect(checkAnswer("norge", "Norge", allNorwegianNames)).toEqual([
    ...
    it("returns match for exact alternative name", () => {
      expect(
        checkAnswer("UK", "Storbritannia", allNorwegianNames, ["UK"]),
      ).toEqual(["match", "UK"]);
      expect(
        checkAnswer("Burma", "Myanmar", allNorwegianNames, ["Burma"]),
      ).toEqual(["match", "Burma"]);
    });
  });
  ...
  describe("fuzzy matches", () => {
    it("returns match for fuzzy main name", () => {
      expect(checkAnswer("Norg", "Norge", allNorwegianNames)).toEqual([
        "match",
        "Norge",
      ]);
      expect(
        checkAnswer("Storbritania", "Storbritannia", allNorwegianNames),
      ).toEqual(["match", "Storbritannia"]);
    });

    it("returns match for fuzzy alternative name", () => {
      expect(
        checkAnswer("Amerik", "De forente stater", allNorwegianNames, [
          "Amerika",
        ]),
      ).toEqual(["match", "Amerika"]);
      expect(
        checkAnswer("Bruma", "Myanmar", allNorwegianNames, ["Burma"]),
      ).toEqual(["match", "Burma"]);
    });
  });
  ...
  describe("no match", () => {
    it("returns no_match for wrong answer", () => {
      expect(checkAnswer("Sverige", "Norge", allNorwegianNames)).toEqual([
        "no_match",
      ]);
      expect(checkAnswer("Japan", "Kina", allNorwegianNames)).toEqual([
        "no_match",
      ]);
    });

    it("returns no_match for too short input", () => {
      expect(checkAnswer("No", "Norge", allNorwegianNames)).toEqual([
        "no_match",
      ]);
      expect(checkAnswer("Ir", "Iran", allNorwegianNames)).toEqual([
        "no_match",
      ]);
    });

    it("returns no_match for gibberish", () => {
      expect(checkAnswer("asdfgh", "Norge", allNorwegianNames)).toEqual([
        "no_match",
      ]);
    });
  });
  ...
  describe("exact match bypasses ambiguity", () => {
    it("exact Sudan for Sudan is match, not ambiguous", () => {
      expect(checkAnswer("Sudan", "Sudan", allNorwegianNames)).toEqual([
        "match",
        "Sudan",
      ]);
    });
    ...
    it("exact Iran for Iran is match, not ambiguous", () => {
      expect(checkAnswer("Iran", "Iran", allNorwegianNames)).toEqual([
        "match",
        "Iran",
      ]);
    });
...
describe("alternativeNames", () => {
  ...
  it("has Amerika and De forente amerikanske stater for United States", () => {
    ...
    expect(alternativeNames["United States"]).toContain(
...
describe("isCloseEnough", () => {
  describe("should accept", () => {
    ...
    it("longer country names with typos", () => {
      expect(
        isCloseEnough(
          "Den sentralafrikanske repubikk",
...
describe("isAmbiguous (with full country list)", () => {
  describe("Mauritius vs Mauritania", () => {
    it("exact matches work", () => {
      expect(isAmbiguous("mauritius", "Mauritius", allNorwegianNames)).toBe(
        false,
      );
      expect(isAmbiguous("mauritania", "Mauritania", allNorwegianNames)).toBe(
        false,
      );
    });
    ...
    it("typing more disambiguates", () => {
      expect(isAmbiguous("mauriti", "Mauritius", allNorwegianNames)).toBe(
        false,
      );
      expect(isAmbiguous("maurita", "Mauritania", allNorwegianNames)).toBe(
        false,
      );
    });
  });
  ...
  describe("Mali vs Malawi vs Malaysia", () => {
    it("exact matches work", () => {
      ...
      expect(isAmbiguous("malaysia", "Malaysia", allNorwegianNames)).toBe(
        false,
      );
    });
  });
  ...
  describe("Slovenia vs Slovakia", () => {
    it("exact matches work", () => {
      expect(isAmbiguous("slovenia", "Slovenia", allNorwegianNames)).toBe(
        false,
      );
      expect(isAmbiguous("slovakia", "Slovakia", allNorwegianNames)).toBe(
        false,
      );
    });
  ...
  describe("India vs Indonesia", () => {
    it("exact matches work", () => {
      ...
      expect(isAmbiguous("indonesia", "Indonesia", allNorwegianNames)).toBe(
        false,
      );
    });
  ...
  describe("Sudan vs Sør-Sudan", () => {
    it("exact matches work", () => {
      ...
      expect(isAmbiguous("sør-sudan", "Sør-Sudan", allNorwegianNames)).toBe(
        false,
      );
    });
    ...
    it("sørsudan without hyphen is NOT ambiguous", () => {
      ...
      expect(isAmbiguous("sørsudan", "Sør-Sudan", allNorwegianNames)).toBe(
        false,
      );
    });
  ...
  describe("Nord-Korea vs Sør-Korea", () => {
    it("exact matches work", () => {
      expect(isAmbiguous("nord-korea", "Nord-Korea", allNorwegianNames)).toBe(
        false,
      );
      expect(isAmbiguous("sør-korea", "Sør-Korea", allNorwegianNames)).toBe(
        false,
      );
      expect(isAmbiguous("nordkorea", "Nord-Korea", allNorwegianNames)).toBe(
        false,
      );
      expect(isAmbiguous("sørkorea", "Sør-Korea", allNorwegianNames)).toBe(
        false,
      );
    });
  ...
  describe("Guinea variations", () => {
    it("exact matches work", () => {
      ...
      expect(
        isAmbiguous("guinea-bissau", "Guinea-Bissau", allNorwegianNames),
      ).toBe(false);
    });
  });
  ...
  describe("unique countries are never ambiguous", () => {
    ...
    it("australia is unique", () => {
      expect(isAmbiguous("australia", "Australia", allNorwegianNames)).toBe(
        false,
      );
    });
  });
...
describe("isCloseEnough - more realistic misspellings", () => {
  ...
  describe("should reject", () => {
    ...
    it("partial country names too short", () => {
      ...
      expect(isCloseEnough("Kongo", "Den demokratiske republikken Kongo")).toBe(
        false,
      );
    });
...
describe("alternative names", () => {
  ...
  it("should accept fuzzy matches on alternative names", () => {
    ...
    expect(
      isCloseEnough(
        "De forente amerikanske statene",
        "De forente amerikanske stater",
      ),
    ).toBe(true);
  });
